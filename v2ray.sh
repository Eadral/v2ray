...
domain_check() {
	...
	test_domain=$(curl -sH 'accept: application/dns-json' "https://cloudflare-dns.com/dns-query?name=$new_domain&type=AAAA" | grep -oE "([0-9,a-f]{1,4}:){1,7}[0-9,a-f]{1,4}" | head -1)
